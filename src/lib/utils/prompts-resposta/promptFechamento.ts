/**
 * Geração de prompt de resposta para status "Fechamento"
 */
...
/**
 * Gera o prompt de resposta para contatos com status "Fechamento"
 ...
 */
export async function generatePromptFechamento(
  contatoId: string,
  ...
  try {
    ...
    let prompt = `status: Fechamento\n\n`;
    prompt += `quantidade de mensagens: ${quantidadeMensagens + 1}\n\n`;
    prompt += `Nesso momento o cliente ja esta interessado em adquirir um produto ou serviço. E aqui devemos informar o cliente que será feito contato com o setor de desenvolvimento, passando um orçamento completo para a implementação do projeto. E também será discutido os detalhes para que o projeto fique perfeito para o cliente\n\n`;
    prompt += `REGRAS\n`;
    prompt += `Se o cliente demonstrou interesse em estar fazendo a aquisição do serviço ou produto, então voce deve informar que o pedido sera encaminhado para o setor de desenvolvimento. Que em breve entrará em contato.\n\n`;
    ...
    console.error('❌ Erro ao gerar prompt Fechamento:', error);
    throw error;
  }
}
