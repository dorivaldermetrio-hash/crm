/**
 * Incrementa o contador de mensagens do status do contato do Instagram DM
 */
...
/**
 * Incrementa o contador de mensagens do status atual do contato do Instagram DM
 */
export async function incrementStatusMessageCountInstagram(
  contatoId: string,
  status: string
  ...
  try {
    ...
    console.log(`✅ Contador ${fieldName} incrementado para contato Instagram ${contatoId}`);
    return { success: true };
  } catch (error) {
    console.error('❌ Erro ao incrementar contador de mensagens do Instagram:', error);
